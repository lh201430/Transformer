...
case class PEConfig(depth:Int,Reg_WIDTH:Int){
//
}
...
class Tile (Tile_Size: Int, dataWidthIn: Int, dataWidthOut: Int,peConfig:PEConfig)extends Component {
  ...
  for (i <- 1 to Tile_Size-1) {
    ...
    PEArry(i)(j).io.vaild <>(io.a_Valid(i)&Delay(io.b_Valid(0),i))// (io.a_Valid(i)&Delay(io.b_Valid(0),i))//Delay(io.a_Valid(i),i)&Delay(io.b_Valid(0),i))//io.a_Valid(i)
...
//
//  val result = new PEresult(dataWidthIn,64,peConfig.depth)
//  for (i <- 0 to Tile_Size - 1) {
//        result.io.input(i).payload <> io.PE_OUT(i)(7 downto 0).asUInt
//        result.io.input(i).valid := io.resultVaild(i)
//    }
//    result.io.mData.ready:=True
//    result.io.mData.valid
