...
import wa.{ForLoopCounter, WaCounter}
...
class PE(A_WIDTH: Int, B_WIDTH: Int,OUT_WIDTH:Int,peConfig:PEConfig) extends Component{
  ...
  val finishCnt = ForLoopCounter(RegNext(RegNext(io.vaild)),16,io.signCount-1)
  io.finish:=finishCnt.valid

  when(finishCnt.valid){
    io.PE_OUT:=reg1
    reg1:=io.vaild?(dsp.io.P) | S(0,peConfig.Reg_WIDTH bits)   //U(0,4 bits)@@(dsp.io.P.resized)
